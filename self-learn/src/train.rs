    ...
    T: GameAnalyzer<Action = A, State = S, Value = V> + Send,
{
    ...
    let num_max_moving_window_percentage_games = number_of_games_per_net
        + (num_games_since_beginning as f32 * options.max_moving_window_percentage) as usize;
    let num_games = std::cmp::min(
        num_max_moving_window_percentage_games,
        options.moving_window_size,
    );
    ...
    let positions_metrics = metric_iter.take(num_games).flat_map(|m| {
        ...
        positions_metrics
            ...
            .into_iter()
            .map(move |(mut sample, move_number)| {
