...
impl Tuner {
        ...
        T: GameAnalyzer<Action = A, State = S, Value = M::Value> + Send,
    {
        ...
        crossbeam::scope(move |s| {
            ...
            let games_to_play =
                generate_games_to_play(&players, options.num_players, options.num_rounds);
            let games_to_play = games_to_play.iter().map(|players| {
                players
                    .iter()
                    .map(|(p_id, p)| {
            ...
            s.spawn(move |_| -> Result<()> {
                ...
                let mut player_scores: Vec<_> =
                    players.iter().map(|(id, m)| (*id, *m, 0.0)).collect();
                ...
                let player_scores = player_scores
                    .iter()
                    .map(|(_, player, score)| PlayerScore {
                        player,
                        score: *score,
                    })
                    .collect::<Vec<_>>();
        ...
        T: GameAnalyzer<Action = A, State = S> + Send,
    {
        ...
        while game_engine.is_terminal_state(&state).is_none() {
            ...
            player_to_move_mcts
                .search_visits(*player_to_move_visits)
                .await?;
        ...
        let scores: Vec<_> = players
            .iter()
            .enumerate()
            .map(|(i, Player { id, .. })| (*id, final_score.get_value_for_player(i + 1)))
            .collect();
...
fn generate_games_to_play<'a>(
    ...
    let games_to_play: Vec<Vec<_>> =
        std::iter::repeat_with(|| players.iter().permutations(num_players))
            .take(num_rounds)
            .flatten()
            .map(|players| {
                players
