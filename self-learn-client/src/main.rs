...
async fn main() -> Result<()> {
    ...
    if let Some(matches) = matches.subcommand_matches("init") {
        ...
        let (model_1_num, model_2_num) =
            update_self_evaluate_options_from_matches(&mut options, matches)?;
        let model_1_num: Option<usize> =
            model_1_num.map(|v| v.parse().expect("Model number not a valid int"));
        let model_2_num: Option<usize> =
            model_2_num.map(|v| v.parse().expect("Model number not a valid int"));
        ...
        let model_nums = models_or_ranges
            .iter()
            .map(|s| {
        ...
        let model_num: Option<usize> =
            model_num.map(|v| v.parse().expect("Model number not a valid int"));
...
fn run_connect4(
    run_name: &str,
    self_learn_options: &SelfLearnOptions,
    self_evaluate_options: &SelfEvaluateOptions,
) -> Result<()> {
    let model_factory = Connect4ModelFactory::new();
...
fn evaluate_connect4(
    run_name: &str,
    model_1_num: Option<usize>,
    model_2_num: Option<usize>,
    ...
    SelfEvaluate::evaluate(&[model_1, model_2], &game_engine, options)?;

    Ok(())
}

fn tournament_connect4(
    run_name: &str,
    models: &[usize],
    ...
    Tournament::tournament(&models, &game_engine, options)?;
...
async fn play_connect4(
    ...
    Play::play(&model, &game_engine, options).await?;
...
fn tournament_quoridor(
    ...
    let models = models
        ...
        .map(|model_num| {
            let model_info =
                ModelInfo::new(QUORIDOR_NAME.to_owned(), run_name.to_owned(), *model_num);
            model_factory.get(&model_info)
        })
        .collect::<Vec<_>>();
...
fn tournament_arimaa(run_name: &str, models: &[usize], options: &TournamentOptions) -> Result<()> {
    ...
    let models = models
        ...
        .map(|model_num| {
            let model_info =
                ModelInfo::new(ARIMAA_NAME.to_owned(), run_name.to_owned(), *model_num);
            model_factory.get(&model_info)
        })
        .collect::<Vec<_>>();
...
fn update_self_learn_options_from_matches(
    options: &mut SelfLearnOptions,
    matches: &clap::ArgMatches,
) -> Result<()> {
    if let Some(number_of_games_per_net) = matches.value_of("number_of_games_per_net") {
        options.number_of_games_per_net = number_of_games_per_net.parse()?
    };
    if let Some(self_play_batch_size) = matches.value_of("self_play_batch_size") {
        options.self_play_batch_size = self_play_batch_size.parse()?
    };
    if let Some(moving_window_size) = matches.value_of("moving_window_size") {
        options.moving_window_size = moving_window_size.parse()?
    };
    if let Some(max_moving_window_percentage) = matches.value_of("max_moving_window_percentage") {
        options.max_moving_window_percentage = max_moving_window_percentage.parse()?
    };
    if let Some(position_sample_percentage) = matches.value_of("position_sample_percentage") {
        options.position_sample_percentage = position_sample_percentage.parse()?
    };
    if let Some(train_ratio) = matches.value_of("train_ratio") {
        options.train_ratio = train_ratio.parse()?
    };
    if let Some(train_batch_size) = matches.value_of("train_batch_size") {
        options.train_batch_size = train_batch_size.parse()?
    };
    if let Some(epochs) = matches.value_of("epochs") {
        options.epochs = epochs.parse()?
    };
    if let Some(learning_rate) = matches.value_of("learning_rate") {
        options.learning_rate = learning_rate.parse()?
    };
    if let Some(max_grad_norm) = matches.value_of("max_grad_norm") {
        options.max_grad_norm = max_grad_norm.parse()?
    };
    if let Some(policy_loss_weight) = matches.value_of("policy_loss_weight") {
        options.policy_loss_weight = policy_loss_weight.parse()?
    };
    if let Some(value_loss_weight) = matches.value_of("value_loss_weight") {
        options.value_loss_weight = value_loss_weight.parse()?
    };
    ...
    if let Some(fast_visits) = matches.value_of("fast_visits") {
        options.fast_visits = fast_visits.parse()?
    };
    if let Some(full_visits_probability) = matches.value_of("full_visits_probability") {
        options.full_visits_probability = full_visits_probability.parse()?
    };
    if let Some(fpu) = matches.value_of("fpu") {
        options.fpu = fpu.parse()?
    };
    if let Some(fpu_root) = matches.value_of("fpu_root") {
        options.fpu_root = fpu_root.parse()?
    };
    if let Some(logit_q) = matches.value_of("logit_q") {
        options.logit_q = logit_q.parse()?
    };
    if let Some(cpuct_base) = matches.value_of("cpuct_base") {
        options.cpuct_base = cpuct_base.parse()?
    };
    if let Some(cpuct_init) = matches.value_of("cpuct_init") {
        options.cpuct_init = cpuct_init.parse()?
    };
    if let Some(cpuct_root_scaling) = matches.value_of("cpuct_root_scaling") {
        options.cpuct_root_scaling = cpuct_root_scaling.parse()?
    };
    if let Some(epsilon) = matches.value_of("epsilon") {
        options.epsilon = epsilon.parse()?
    };

    Ok(())
}

fn update_model_options_from_matches(
    options: &mut ModelOptions,
    matches: &clap::ArgMatches,
) -> Result<()> {
    if let Some(number_of_filters) = matches.value_of("number_of_filters") {
        options.number_of_filters = number_of_filters.parse()?
    };
    if let Some(number_of_residual_blocks) = matches.value_of("number_of_residual_blocks") {
        options.number_of_residual_blocks = number_of_residual_blocks.parse()?
    };

    Ok(())
}

fn update_self_evaluate_options_from_matches(
    options: &mut SelfEvaluateOptions,
    matches: &clap::ArgMatches,
    ...
    if let Some(num_games) = matches.value_of("num_games") {
        options.num_games = num_games.parse()?
    };

    Ok((
        matches.value_of("model_1").map(|s| s.to_owned()),
        matches.value_of("model_2").map(|s| s.to_owned()),
    ))
}

fn update_play_options_from_matches(
    options: &mut PlayOptions,
    matches: &clap::ArgMatches,
...
fn update_tournament_options_from_matches(
    ...
    Ok(matches
        .value_of("models")
        .ok_or_else(|| anyhow!("model ranges not specified"))?
        .to_owned())
}
...
fn initialize_directories_and_files(
    game_name: &str,
    run_name: &str,
    options: &Options,
) -> Result<PathBuf> {
    ...
    let serialized_options =
        serde_json::to_string_pretty(options).expect("Unable to serialize options");
    file.write_all(serialized_options.as_bytes())
        .expect("Unable to write options to file");
