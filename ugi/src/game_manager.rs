...
use model::Analyzer as AnalyzerTrait;
...
pub struct GameManager<S, A, U, E, M> {
...
impl<S, A, U, E, M> GameManager<S, A, U, E, M> {
    ...
    pub async fn command(&self, command: UGICommand<S, A>) {
        match command {
            UGICommand::UGI => {
                self.output_cmd("protocol-version", "1");
                self.output_cmd("id name", NAME);
                self.output_cmd("id author", AUTHOR);
                self.output_cmd("ugiok", "");
            }
            UGICommand::IsReady => {
                self.output_cmd("readyok", "");
            }
    ...
    fn output_cmd(&self, cmd_name: &str, cmd_value: &str) {
        self.output_tx
            .send(Output::Command(cmd_name.to_string(), cmd_value.to_string()));
    }

    fn output_debug(&self, msg: &str) {
        self.output_tx.send(Output::Debug(msg.to_string()));
    }

    fn output_info(&self, msg: &str) {
        self.output_tx.send(Output::Info(msg.to_string()));
    }

    fn output_info_val(&self, msg: &str, value: &str) {
        self.output_tx
...
where
    ...
    U: InitialGameState<State = S> + ActionsToMoveString<State = S, Action = A> + Sync,
    E: GameEngine<State = S, Action = A> + ValidActions<State = S, Action = A> + Sync,
    M: AnalyzerTrait<State = S, Action = A, Value = E::Value> + Sync,
{
    pub fn new(ugi_mapper: U, engine: E, model: M) -> (Self, mpsc::UnboundedReceiver<Output>) {
        ...
        let game_manager = Self {
            ...
            options: Arc::new(Mutex::new(init_options())),
            ponder_active: Arc::new(AtomicBool::new(false)),
            ugi_mapper,
            engine,
            model,
        };

        game_manager.init_game_loop(command_rx, output_tx);
    ...
    fn init_game_loop(
        ...
        thread::spawn(move || {
            handle.block_on(async move {
                ...
                let mut game_state: <U as InitialGameState>::State =
                    self.ugi_mapper.initial_game_state();
                ...
                while let Some(command) = command_rx.recv().await {
                    ...
                    match command {
                        CommandInner::NewGame => {
                            let option =
                                UGIOption::InitialGameState(self.ugi_mapper.initial_game_state());
                        ...
                        CommandInner::Ponder => {
                            ...
                            if options_lock.visits == 0 {
                                ...
                                self.output_info("ponder started");
                                mcts.search(|visits| {
                                ...
                                self.output_info("skipping ponder as options are fixed visits.");
                            }
                        }
                        ...
                        CommandInner::FocusActions(actions) => {
                            for action in actions {
                                ...
                                focus_game_state =
                                    self.engine.take_action(&focus_game_state, &action);
                        ...
                        CommandInner::Go => {
                                ...
                                && self.engine.terminal_state(&focus_game_state).is_none()
                            {
                                let depth = if options.visits != 0 {
                                    self.output_info(&format!("search visits: {}", options.visits));
                            ...
                            self.output_info_val("pv", &move_string);
                            self.output_info_val(
                                "time",
                                &search_start.elapsed().as_secs().to_string(),
                            );
                            self.output_info_val(
                                "root_score",
                            ...
                            self.output_info_val(
                                "score",
                                &format!("{:.3}", scores.last().unwrap_or(&0.5)),
                            );
                            self.output_info_val(
                                "moves_left",
                            ...
                            self.output_info_val(
                                "visits",
                                &visits.iter().max().unwrap_or(&0).to_string(),
                            );
                            self.output_info_val(
                                "depth",
                                &depths.iter().max().unwrap_or(&0).to_string(),
                            );
                            ...
                            self.output_cmd("bestmove", &move_string)
                        }
                    }
                }
            });
